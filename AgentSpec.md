...
Deployment is done via the included `deploy_agent.py`, which packages `linear_agent` and `requirements.txt` as inline source packages (no Cloud Storage bucket required) for the `mapache-app` project.
